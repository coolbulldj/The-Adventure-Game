...
class UIScrollingFrame:
    def __init__(self, x, y, width, height, canvas_height):
        ...
        self.sb_rect = pygame.Rect(self.rect.right - self.sb_thickness, self.rect.y, self.sb_thickness, self.rect.height)
        ...
        self.canvas_surface = pygame.Surface((width - self.sb_thickness, self.canvas_height))

    def update_scrollbar(self):
        """Calculates the dynamic size and position of the scroll bar thumb."""
        visible_ratio = self.rect.height / self.canvas_height
        self.handle_height = max(20, int(self.rect.height * visible_ratio)) # Ensure handle is clickable
        
        # Map canvas position directly to scrollbar screen tracks
        scroll_ratio = self.canvas_y / (self.canvas_height - self.rect.height) if (self.canvas_height - self.rect.height) > 0 else 0
        handle_y = self.rect.y + int(scroll_ratio * (self.rect.height - self.handle_height))
        
        self.handle_rect = pygame.Rect(self.rect.right - self.sb_thickness, handle_y, self.sb_thickness, self.handle_height)
    ...
    def handle_event(self, event):
        ...
        # 4. Active Motion Translation while dragging
            ...
            scroll_ratio = (new_handle_y - min_y) / track_length if track_length > 0 else 0
    ...
    def draw(self, dest_surface, content_render_func):
        ...
        pygame.draw.rect(self.frame_surface, self.scrollbar_bg, 
                         (self.rect.width - self.sb_thickness, 0, self.sb_thickness, self.rect.height))
        
        # Render dynamic visual indicator track handle
        color = self.scrollbar_active_color if self.is_dragging else self.scrollbar_handle_color
        pygame.draw.rect(self.frame_surface, color, 
                         (self.rect.width - self.sb_thickness, self.handle_rect.y - self.rect.y, self.sb_thickness, self.handle_height))
